...
llm_build_qwen35moe::llm_build_qwen35moe(const llama_model & model, const llm_graph_params & params) :
    ...
    if (hparams.nextn_predict_layers > 0 && n_outputs == n_tokens && cparams.mtp_enabled) {
        ...
        const int n_chain = 0; // Disabled: chain is a closed-loop AR predictor that sees only its own K/V,
                               // creating self-reinforcing token loops (degenerate output).
                               // Base MTP prediction is sufficient for speculative decoding.
